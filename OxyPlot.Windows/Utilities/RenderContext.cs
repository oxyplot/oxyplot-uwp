...
namespace OxyPlot.Windows
{
    ...
    public class RenderContext : IRenderContext
    {
        ...
        private readonly Stack<OxyRect> clipStack;
        ...
        public RenderContext(Canvas canvas)
        {
            ...
            this.clipStack = new Stack<OxyRect>();
        }
        ...
        /// <summary>
        /// Gets the number of clipped.
        /// </summary>
        public int ClipCount => this.clipStack.Count;
        ...
        /// <param name="edgeRenderingMode">The rendering mode.</param>
        ...
        public void DrawLine(IList<ScreenPoint> points, OxyColor stroke, double thickness, EdgeRenderingMode edgeRenderingMode, 
            double[] dashArray = null, LineJoin lineJoin = LineJoin.Miter)
        {
            ...
            this.SetStroke(e, stroke, thickness, lineJoin, dashArray, false);
        ...
        /// <param name="dashArray">The dash array.</param>
        /// <param name="lineJoin">The line join type.</param>
        public void DrawLineSegments(IList<ScreenPoint> points, OxyColor stroke, double thickness,
            EdgeRenderingMode edgeRenderingMode, double[] dashArray = null, LineJoin lineJoin = LineJoin.Miter)
        {
            ...
            this.SetStroke(path, stroke, thickness, lineJoin, dashArray);
            ...
            for (int i = 0; i + 1 < points.Count; i += 2)
            {
                ...
                // continue;
                // }
                Point pnt1 = new Point(points[i].X, points[i].Y);
                Point pnt2 = new Point(points[i + 1].X, points[i + 1].Y);
                var figure = new PathFigure { StartPoint = pnt1, IsClosed = false };
                figure.Segments.Add(new LineSegment { Point = pnt2 });
        ...
        /// <param name="dashArray">The dash array.</param>
        /// <param name="lineJoin">The line join type.</param>
        public void DrawPolygon(IList<ScreenPoint> points, OxyColor fill, OxyColor stroke, 
            double thickness, EdgeRenderingMode edgeRenderingMode, double[] dashArray = null, LineJoin lineJoin = LineJoin.Miter)
        {
            var po = new Polygon
            {
                CompositeMode = ElementCompositeMode.SourceOver
            ...
            this.SetStroke(po, stroke, thickness, lineJoin, dashArray);
        ...
        /// <param name="dashArray">The dash array.</param>
        /// <param name="lineJoin">The line join type.</param>
        public void DrawPolygons(IList<IList<ScreenPoint>> polygons, OxyColor fill, OxyColor stroke, 
            double thickness, EdgeRenderingMode edgeRenderingMode, double[] dashArray = null, LineJoin lineJoin = LineJoin.Miter)

        {
            ...
            foreach (var polygon in polygons)
            {
                ...
                foreach (var p in polygon)
                {
                    ...
                    if (first)
                    {
                        figure.StartPoint = pnt;
                        first = false;
                    }
                    else
                    {
                        figure.Segments.Add(new LineSegment { Point = pnt });
                    }
                }
        ...
        /// <param name="stroke">The stroke color.</param>
        /// <param name="thickness">The stroke thickness.</param>
        /// <param name="edgeRenderingMode">Rendering mode.</param>
        public void DrawRectangle(OxyRect rectangle, OxyColor fill, OxyColor stroke, 
            double thickness, EdgeRenderingMode edgeRenderingMode)
        {
            ...
            if (stroke.IsVisible())
            {
                Color col = Color.FromArgb(stroke.A, stroke.R, stroke.G, stroke.B);
                el.Stroke = new SolidColorBrush(col);
                el.StrokeThickness = thickness;
            }

            if (fill.IsVisible())
            {
                Color col = Color.FromArgb(fill.A, fill.R, fill.G, fill.B);
        ...
        /// <param name="thickness">The stroke thickness.</param>
        /// <param name="edgeRenderingMode">The rendering mode.</param>
        public void DrawRectangles(IList<OxyRect> rectangles, OxyColor fill, OxyColor stroke, 
            double thickness, EdgeRenderingMode edgeRenderingMode)
        {
            var path = new Path
            {
            ...
            foreach (var rect in rectangles)
            {
                Rect r = rect.ToRect(false);
                gg.Children.Add(new RectangleGeometry { Rect = new Rect(r.X, r.Y, r.Width, r.Height) });
            }
        ...
        {
            Rect rect = clippingRect.ToRect(false);
            this.clipRect = new Rect(rect.X, rect.Y, rect.Width, rect.Height);
            this.clip = true;
            return true;
        }
        ...
        private static FontWeight GetFontWeight(double fontWeight)
        {
            ...
            if (fontWeight > OxyPlot.FontWeights.Normal)
            {
                fw = new FontWeight() { Weight = (ushort)FontWeights.Bold };
            }
            else
            {
                fw = new FontWeight() { Weight = (ushort)FontWeights.Normal };
            }
            return fw;
        }
        ...
        public void PushClip(OxyRect clippingRectangle)
        {
            ...
            {
                OxyRect currentClippingRectangle = this.clipStack.Peek();
                OxyRect newClippingRectangle = clippingRectangle.Intersect(currentClippingRectangle);
        ...
        public void PopClip()
        {
            ...
            OxyRect currentClippingRectangle = this.clipStack.Pop();
            if (this.clipStack.Count > 0)
            {
                OxyRect newClippingRectangle = this.clipStack.Peek();
